...
//! - not all iterators are logged, *only* `IntoParallelRefIterator`. You can however log
//! any iterator `i` by turning it into a `Logged` iterator using `Logged::new(i)`.
//! - `collect` is not logged.
